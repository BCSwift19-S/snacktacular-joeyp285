...
class SpotsListViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        spots.spotArray.append(Spot(name: "Shake Shack", address: "Chestnut Hill St.", coordinate: CLLocationCoordinate2D(), averageRating: 0.0, numberOfReviews: 0, postingUserID: "", documentID: ""))
...
extension SpotsListViewController: FUIAuthDelegate {
    ...
    func authUI(_ authUI: FUIAuth, didSignInWith user: User?, error: Error?) {
        if let user = user {
            tableView.isHidden = false
            print("*** We signed in with user \(user.email ?? "unkown e-mail")")
        }
    }
